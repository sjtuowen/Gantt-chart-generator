    ...
    for idx, row in df.iterrows():
        ...
        if not pd.isna(row['start']) and not pd.isna(row['end']):
            if row['end'] < row['start']:
                errors.append(f"Task '{task_name}': End date cannot be earlier than start date.")
        
        if not pd.isna(row.get('progress')):
            progress = row['progress']
            if progress < 0 or progress > 100:
                errors.append(f"Task '{task_name}': Progress must be between 0 and 100.")
        
        if row.get('status') not in ['done', 'active', 'pending', 'critical', 'milestone']:
